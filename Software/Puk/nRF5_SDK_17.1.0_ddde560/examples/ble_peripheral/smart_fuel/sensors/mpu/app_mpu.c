...
uint32_t app_mpu_enter_sleep_mode() 
{
    uint8_t *data = 64; // Bit Mask for going in sleep mode
    nrf_drv_mpu_write_single_register(MPU_REG_PWR_MGMT_1, *data);
}

uint32_t app_mpu_wake_up() 
{
    uint8_t *data = 8; // Bit Mask for going in sleep mode
    nrf_drv_mpu_write_single_register(MPU_REG_PWR_MGMT_1, *data);
}
